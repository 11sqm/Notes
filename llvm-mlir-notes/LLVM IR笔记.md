...
### (2) 寄存器和栈
由于CPU中通用寄存器数量有限，因此需要将一部分数据放在内存中，所以需要使用栈。此外，由于需要操作内存地址，但寄存器没有通用的内存表示，所以只能把数据放在栈上来完成对地址的操作。

如果寄存器的数量足够，并且代码中没有需要操作内存地址的时候，寄存器是足够胜任的，并且更加高效的。

#### [1] 寄存器
LLVM IR引入了虚拟寄存器的概念。在LLVM IR中，一个函数的局部变量可以是寄存器或者栈上的变量。对于寄存器而言，只需要像普通赋值语句一样操作，但是名字必须以 `%` 开头：
```llvm
%local_variable = add i32 1, 2
```
此时，`%local_variable` 代表一个寄存器，值为 `1` 和 `2` 相加结果。

在不同ABI下，有 `callee-saved register` 和 `caller-saved register`。简单来说，在函数内部，某些寄存器的值不能改变，换言之，在函数返回时，某些寄存器的值要和进入函数前相同。比如，在System V的ABI下，`rbp`, `rbx`, `r12`, `r13`, `r14`, `r15` 都需要满足这以条件，这在System V的ABI下被称作**callee-saved registe**。由于LLVM IR面向多平台，因此其内部自动完成分配，当所有没有呗保留的寄存器全部被使用时，则LLVM IR会将这些被保留的寄存器放在栈上，然后继续使用这些被保留的寄存器。当函数退出时，会自动从栈上获取到相应的值放回寄存器内。

如果所有通用寄存器都被使用，则LLVM IR会将剩余的值放在栈上，但对于用户而言，实际上都是虚拟寄存器。

因此LLVM IR对于寄存器的使用可以粗略概括为：
...
如果在调用别的函数的过程中，如果调用方的非callee-saved register中存有一些后续需要用到的数据，需要将这些数据放入栈上，在函数调用结束后，再从栈上将这些值放回相应的寄存器中。

#### [2] 栈
当需要操作地址以及需要可变变量时，需要使用栈。LLVM IR对栈的使用十分简单，直接使用 `alloca` 指令即可。如：
```llvm
...
这两个变量实际上都是 `ptr` 指针，指向所处的一个 `i32` 大小内存区域。

因此，以下操作是错误的，因为 `@global_variable` 只是一个指针：
```llvm
%1 = add i32 1, @global_variable ; Wrong!
```

如果要操作值，必须使用 `load` 和 `store` 命令。因此，获取 `@global_variable` 的值，需要：
```llvm
%1 = load i32, ptr @global_variable
```
该指令把一个 `ptr` 指针 `@global_variable` 的 `i32` 类型的值赋给虚拟寄存器 `%1`。

类似的，如果要将值存储到全局变量或栈上变量里，会需要 `store` 命令：
```llvm
