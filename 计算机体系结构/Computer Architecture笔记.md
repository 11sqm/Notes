...
##### [1] 数据复制
![DRAM数据复制](pictures/DRAM数据复制.png)
对于一个DRAM子阵列，源行和目标行位于同一个子阵列中。首先激活源行，将数据带入行缓冲区。其次激活目标行，这隐含地停用源行并将缓冲区连接到目标行。通过两个连续激活，则将源行从其位置带到行缓冲区，并将其写入目标行。其具体电路原理如下图所示：
![DRAM复制电路图](pictures/DRAM数据复制电路图.png)

对于位于不同bank的数据，无法通过上述违反时许约束实现数据复制。可以通过更改DRAM芯片命令，以便利用共享内部总线，需要说明的是DRAM本身不具有库内通信。
![inter-bank数据移动](pictures/inter-bank数据移动.png)

考虑位于同一bank不同子阵列情况，此时如果想将数据从一个子阵列复制到另一个子阵列，必须通过内，导致较长执行时间。**本质上，DRAM中的低连接性是大数据移动的基本瓶颈。**
![subarray数据移动](pictures/subarray数据移动.png)

一个解决思路是通过新的羁绊，实现子阵列之间的广泛连接，并尽可能减少修改。Low-cost Inter-linked subarrays (LISA) 思路是通过添加隔离晶体管，将子阵列相互连接。
![LISA](pictures/LISA.png)
按照这一思路，可以将子阵列进一步拓展互连，拥有一条连接的长位线。但如果路径太长，延迟可能会很长，可靠性也可能受到影响，所以一般限制传输最多到两个或三个子阵列，具体分析可以参考原论文。

##### [2] In-Memory Computation
在DRAM中，可以通过激活行实现一定与或。
![In-DRAM布尔逻辑](pictures/In-DRAM布尔逻辑.png)
上述电路中，同时激活三行，根据电荷共享原理，电路级别上，如果至少两个单元被充电，则位线扰动为正，最终在位线上得到正的电源电压。同理，如果至少两个单元被放电，则位线扰动为负，最终在位线上得到零电压。显然上述电路实现了布尔函数`AB+BC+AC`。

为在内存中进行类似与或运算，同时避免实现任意多行的行解码器。所以往往是在子阵列中指定一个特殊位置，通过克隆操作将数据移动至该位置，并只激活该部分。
![In-DRAM与](pictures/In-DRAM与.png)

然而逻辑非实现与上述与或实现有所不同，需要对DRAM电路有所改动。
![In-DRAM非电路](pictures/In-DRAM非电路.png)
