...
DiT本质是一种新型的扩散模型，结合了去噪扩散概览模型(DDPM)和Transformer架构。

其核心思想在于，使用Transformer作为扩散模型骨干网络，而非传统的卷积神经网络，以处理图像的潜在表示。

### 1. DiT原理
DiT架构基于Latent Diffusion Model (LDM) 框架，采用Vision Transformer (ViT) 作为主干网络，并通过调整ViT的归一化来构建可拓展的扩散模型。
...
![DiT三种变种形式](pictures/DiT三种变种形式.png)
